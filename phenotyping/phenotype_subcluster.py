...
def phenotype_subcluster (adata, cluster_group = None, small_pop= 50, too_big_factor=0.4, group_others= False, k= 10, n_pcs=20, method = 'kmeans', resolution=1, clustering_metric='euclidean', nearest_neighbors= 30, label='phenotype', use_raw_data = True, random_state=0):
    """
    Parameters
    ----------
    ...
    def k_clustering (pheno, adata, k, label, use_raw_data, random_state):
        ...
        kmeans = MiniBatchKMeans(n_clusters=k, random_state=random_state).fit(data_subset)
    ...
    if cluster_group is not None:
        ...
        pheno = adata.obs[label].unique()
    
    # Run the specified method
    if method == 'kmeans':
        # Apply the Kmeans function
        r_k_clustering = lambda x: k_clustering(pheno=x, adata=adata, k=k, label=label, use_raw_data=use_raw_data, random_state=random_state) # Create lamda function 
        all_cluster_labels = list(map(r_k_clustering, pheno)) # Apply function 
    if method == 'phenograph':
        r_phenograph_clustering = lambda x: phenograph_clustering(pheno=x, adata=adata, primary_metric=clustering_metric, nearest_neighbors=nearest_neighbors) # Create lamda function 
        all_cluster_labels = list(map(r_phenograph_clustering, pheno)) # Apply function        
    if method == 'leiden':
        r_leiden_clustering = lambda x: leiden_clustering(pheno=x, adata=adata, nearest_neighbors=nearest_neighbors, n_pcs=n_pcs, resolution=resolution) # Create lamda function 
        all_cluster_labels = list(map(r_leiden_clustering, pheno)) # Apply function 
    if method == 'parc':
        r_parc_clustering = lambda x: parc_clustering(pheno=x, adata=adata, random_state=random_state,resolution=resolution,too_big_factor=too_big_factor,small_pop=small_pop) # Create lamda function 
    ...
    sub_clusters = pd.DataFrame(adata.obs[label]).merge(sub_clusters, how='outer', left_index=True, right_index=True)
